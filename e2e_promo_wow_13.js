//Deal is wallet applicable, user has marketing credit only. Can only use either marketing or refundable credit.
var config = require('./e2e_promo_config.js');

describe("Wow-13. Deal is wallet applicable, user has marketing credit only", function() {
    ...
    it("Login to Wowcher, check marketing credit box, then apply promocode", function() {
        ...
        element(by.name('email')).sendKeys(config.user01);
        ...
        element(by.xpath('//*[@id="app"]/cmp-container/div/div/cmp-content/main/div/cmp-checkout/div/div[2]/cmp-tile/div/div/main/div/cmp-tile-content[1]/div[2]/div[3]/cmp-promo-code/div/div/p')).getText().then(function (text) {

            console.log(text);
            expect(text).toEqual("Promo code cannot be used with marketing credit");

        });
    });

    it("Login to Wowcher, apply promocode, then check marketing credit box", function() {
        ...
        element(by.css('.checkout__wallet--error.ng-binding.ng-scope')).getText().then(function (text) {
