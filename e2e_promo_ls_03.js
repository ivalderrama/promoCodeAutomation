//Testing by creating a promo code, edit and set it set to Spiked, then trying to apply the promo code to a deal.
var config = require('./e2e_promo_config.js');

describe("Liv-03. Create promocode and set to to spiked, then apply promocode", function() {
    ...
    it("Edit promocode and set it to Spiked", function () {
        ...
        element(by.id('ext-gen253')).click();
        browser.sleep(1000);
        element(by.id('status-combo')).clear().sendKeys('Spiked').sendKeys(protractor.Key.ENTER);
        browser.sleep(1000);
